    ...
    <main>
      ...
      {tab==="subjects" && <>
        ...
        <div className="panel">
          ...
          {subjects.map(s=><div className="subjectRow" key={s.id}>
            <div><strong>{s.name}</strong><span>{s.code || "No code"} · {s.class_type}</span></div>
            <span>{s.weekly_schedule.map(x=>weekdayNames[x]).join(", ") || "No schedule"}</span>
          </div>)}
        </div>
      </>}
